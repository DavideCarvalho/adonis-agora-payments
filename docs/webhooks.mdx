...
<Callout type="warn" title='Breaking change: an empty credential slot used to mean "skip"'>
It used to be `if (webhookToken !== undefined)` — a driver with nothing configured verified
**nothing**, and the mounted `POST /payments/webhook/:provider` accepted any body anyone posted to
it, including one that marks a payment paid. Nothing logged it and nothing warned.

If you are upgrading and a provider's webhook credential is not set, the app now throws at boot
naming the provider and the route it would have mounted. Set the credential (each provider page
names the env var), or say explicitly that verification happens upstream:
...
`'payment.suceeded'` used to register a handler nothing ever called: the processor looked the type
up, missed, and skipped — while the ledger still recorded the delivery as **processed** and the
route still answered `200`, which tells the gateway it never has to send that event again. The
grant simply never happened, and nothing anywhere said so.
...
- Two handlers claiming the same type also throw, naming both files. Registration is a map keyed by
  event type, so the second used to silently replace the first.
