...
<Callout type="warn" title="Existing installs need the new migration">
`external_reference` arrived after `create_billing_tables`, in a second published migration
(`add_billing_external_reference`). Until you run it, the column is not there: the reference is
**not** stored, `findPaymentByExternalReference` answers `null` for every id, and the browser
status endpoint falls back to reading your reference as a gateway id. Nothing else changes, and
nothing is backfilled — a reference that was never stored cannot be recovered from the payload.
</Callout>
