...
| `telescope` | `'auto'` | register the typed Telescope watcher when telescope is registered in the app; `true` throws when it is missing |
...
| `billing.role` | `'all'` | `'api'` mounts the webhook route and never processes; `'worker'` processes and never mounts. Both require `dispatcher` `'durable'` or `'queue'` |
| `billing.store` | the Lucid store | swap the persistence entirely, or keep it with your own models via `billingStores.lucid({ models })` — note that naming a store makes `autoCreateSchema` no longer reach it |
