...
Reconcile is invoice-shaped: it lists a customer's gateway invoices and upserts the **paid** ones.
...
### `payments:health` — the four checks
...
It asks **four** questions, each a failure that produces no error anywhere:
...
Each threshold has a flag: `--stuck-after` and `--unconfirmed-after` in minutes, `--window` (the
failure window) and `--dispute-window` in hours. See [CLI](/docs/payments/cli#paymentshealth).
