...
- **Invoices.** `capabilities.invoices` is `false` and **`listInvoices` throws**. Pix
  ...
  resource at all. Answering `[]` would be indistinguishable from "this customer has no
  invoices" — the reader concludes the customer never bought anything, and nothing in the
  flow says otherwise. `PaymentsManager.assertCapability` already stops the
