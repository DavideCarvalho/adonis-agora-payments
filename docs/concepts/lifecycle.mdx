...
Step 6 is why that redelivery is cheap — the events that succeeded are already `processed` and skip
...
| `payment.refunded` | a refund completed | a known payment row becomes `refunded` |
...
| `payment.updated` | a payment changed in some other way | nothing |
