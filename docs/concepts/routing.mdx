...
The bare call used to skip `config.methods` entirely, and that is the same trap one level up: an app
that configured pix/credit_card/boleto routing and then called `driver()` got a driver bound to
**nothing**, so every charge fell back to the gateway dashboard's default unless the caller happened
to repeat `method:`. Three cases now:
