...
describe('renderLoginPage', () => {
  const html = renderLoginPage('/payments');

  it('keeps the "Sign in — Payments" title', () => {
    expect(html).toContain('<title>Sign in — Payments</title>');
  });
  ...
  it('posts to `<basePath>/login`, as a real form AND from the script', () => {
    expect(html).toContain('<form id="login-form" method="post" action="/payments/login"');
    expect(html).toContain('"/payments/login"');
  });

  it('works without JavaScript: returnTo rides in a hidden field, defaulting to the base path', () => {
    expect(html).toContain('<input type="hidden" name="returnTo" value="/payments" />');
    expect(renderLoginPage('')).toContain('name="returnTo" value="/"');
    const deep = renderLoginPage('/payments', { returnTo: '/payments/runs/42' });
    expect(deep).toContain('name="returnTo" value="/payments/runs/42"');
    expect(deep).toContain(JSON.stringify('/payments/runs/42'));
  });
  ...
  it('refuses an open-redirect returnTo and escapes what it echoes', () => {
    for (const bad of ['//evil.com', 'https://evil.com/x', 'runs', 42, undefined]) {
      expect(renderLoginPage('/payments', { returnTo: bad })).toContain(
        'name="returnTo" value="/payments"',
      );
    }
    const quoted = renderLoginPage('/payments', {
      returnTo: '/payments/x"><script>alert(1)</script>',
    });
    expect(quoted).not.toContain('<script>alert(1)');
    expect(quoted).toContain('value="/payments/x&quot;&gt;&lt;script&gt;alert(1)&lt;/script&gt;"');
  ...
  it('shows the error notice server-side for the no-JS round trip', () => {
    ...
    expect(renderLoginPage('/payments', { error: true })).toContain(
      '<p id="error" role="alert" style="display:block">',
    );
  });

  it('carries the CSP nonce on both the <style> and the <script>', () => {
    const nonced = renderLoginPage('/payments', { nonce: 'n0nce"x' });
  ...
  it('embeds basePath via JSON so a quote in it cannot break out of the script', () => {
    ...
    const injected = renderLoginPage('/pay"ments');
    expect(injected).toContain(JSON.stringify('/pay"ments/login'));
    expect(injected).not.toContain('"/pay"ments/login"');
  });
});
