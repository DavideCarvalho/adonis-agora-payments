...
| Table | Purpose |
| --- | --- |
| `billing_customers` | a polymorphic link table for your users' gateway customers — **not written by the library** (see below) |
...
`billing_customers` is the exception — the migration creates it, but nothing in the library writes
to it. The [Billable mixin](#the-billable-model) keeps the gateway customer id on **your** model,
which is where the library reads it from. The table is there for apps that want a separate
polymorphic mapping (one user, several gateways); if you do not fill it yourself, it stays empty and
`payments:sync --all` finds nothing.
