...
const forbidden: AccessDeniedInfo = { status: 403, reason: 'forbidden', basePath: '/payments' };
...
describe('renderAccessDeniedPage', () => {
  ...
  it('picks copy and buttons from the reason', () => {
    ...
    expect(signIn).toContain('class="btn primary" href="/payments/login"');
  ...
  it('offers "sign in as someone else" on a 403 when a login page exists', () => {
    const html = renderAccessDeniedPage({ ...forbidden, loginHref: '/payments/login' });
