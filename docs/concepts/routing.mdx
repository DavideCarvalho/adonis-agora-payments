...
An explicit method on the charge still wins: routing is a default, not an override. And a driver
resolved by **name** comes back untouched — `driver('stripe')` routed nothing, so there is nothing to
thread.
