...
`POST /payments/webhook/square` accepted any body anyone posted, including one that marks a
...
- With neither configured the driver reports `'unconfigured'` and the app refuses to boot. For
  local development without dashboard setup, name the provider in `allowUnverifiedWebhooks`.
...
**A chargeback withdraws the money before you hear about it.** Square's reference is blunt:
"Square withholds the disputed funds from the seller's Square account balance until the bank
issues a final resolution on the case. If there are insufficient funds in the Square account
balance, the funds are removed (debited) from the seller's most recently linked bank account."
...
<Callout type="warn" title="Square does not say whether an inquiry withholds funds">
