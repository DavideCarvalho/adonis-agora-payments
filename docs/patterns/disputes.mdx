...
<ProviderSummary matrix="disputes" />
...
- **A dispute with no deadline is never in it.** `listDisputesDueWithin` filters on a date, and a
  gateway that sends none produces no row here. That is what the second check is for.
...
3  Disputes still open and unanswered
     ...
     deadline check reports zero forever.
```

`open_disputes` counts every dispute in `warning`, `open` or `under_review`, with **no threshold and
...
The bundled console shows the same data on its Disputes screen, leading with the windows closing
rather than the log; see [Dashboard](/docs/payments/dashboard).
...
[`POST <path>/api/disputes/:gatewayId/resolve`](/docs/payments/dashboard#recording-how-a-dispute-ended),
...
Nothing is sent to the gateway by this. The decision was made at the bank; this writes down which
way it went — and the absent-fields-are-left-alone rule keeps the deadline and the reason the opening
event carried.
