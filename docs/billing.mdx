...
The configure command publishes **three** migrations — `create_billing_tables` and, for installs
that already ran it, `add_billing_external_reference` (the two columns added afterwards) and
`add_billing_disputes` (the table added afterwards). Run all of them (`node ace migration:run`);
on a fresh install the later two find their work already done and say so by doing nothing. Together
they create five tables:
...
four operational checks — both plain functions over the same store.
