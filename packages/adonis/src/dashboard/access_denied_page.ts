 ...
 * mirrors the payments SPA so the refusal reads as part of the same product, not a server default.
 *
 * Ported to `@adonis-agora/durable`, `@adonis-agora/media`, `@adonis-agora/agent` and
...
export interface AccessDeniedInfo {
  ...
  /** The console's mount path (`/payments`; `''` when mounted at the root). */
  basePath: string;
...
export interface AccessDeniedPageOptions {
  /** Console name in the eyebrow and `<title>`. Default: the console's own name (`Payments`). */
  brand?: string;
...
export const CONSOLE = {
  brand: 'Payments',
  accent: '#22d3ee',
  packageName: '@adonis-agora/payments',
} as const;
