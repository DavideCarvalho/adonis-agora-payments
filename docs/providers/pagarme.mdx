...
- **Webhooks**: authenticated by the HTTP Basic credentials you set on the webhook endpoint in
  ...
  driver rejects any request that does not carry them. They are **optional at Pagar.me and required
  here**: with neither set the driver reports `'unconfigured'` and the app refuses to boot, because
  nothing else about a Pagar.me delivery is authenticated.
...
`POST /payments/webhook/pagarme` accepted any body anyone posted, including one that marks a
