...
| `withPayment()` | `gatewayId`, `provider`, `status`, `amount`, `currency`, `customerId`, `subscriptionId`, `externalReference`, `payload`, `createdAt`, `updatedAt`, `paidAt` |
...
const payment = await getPayments().charge({
  customerId: user.billingCustomerId!,
