...
- **Dispute windows closing.** An open chargeback whose evidence deadline is inside the next three days. The check names each one, so the panel links to the dispute rather than to a number.
...
A chargeback has a clock, and it is the only clock in this package that runs against you. The
Disputes screen has two modes over the same table:

- **the log** — every dispute newest first, filterable by status (`warning`, `open`,
  `under_review`, then the resolutions) and by gateway;
- **the work list** — only the OPEN disputes that carry a deadline, ordered by the window that
  closes soonest.

A dispute whose deadline has already passed **stays in the work list**. It is still open and still
unanswered, and dropping it the moment it expires would empty the screen at exactly the moment it
became urgent. A dispute whose gateway sent no deadline at all shows the deadline as unknown rather
than as distant — the two are not the same, and only one of them is safe to ignore.

<Callout title="There is no Fight button, on purpose">
Whether to submit evidence or simply refund is a business rule: it turns on the dispute fee, the
evidence your app actually holds, and the chargeback ratio that puts a merchant into a card
network's monitoring programme — which is why refunding a dispute you would have won is sometimes
right. That decision belongs in your code, against your data, not behind a console button. This
screen exists so nobody misses the window; submitting evidence is
