...
Pagar.me's dispute *lifecycle* (evidence submitted, won, lost) is not on the webhook at
all: it lives in a separate Stone-hosted Disputes API (`GET
https://api.stone.com.br/v1/disputes`, its own static key), which this driver does not
speak. The contract has no resolution event either, by design.

<Callout type="warn" title="charge.chargedback is deprecated — migrate by 2026-09-30">
Pagar.me's event list marks `charge.chargedback` as being replaced by **`chargeback.received`**,
with a migration deadline of 30/09/2026. The driver does **not** map `chargeback.received`
yet: its payload shape is undocumented, and `data.id` there may well be a chargeback id
rather than a charge id — filing a dispute under the wrong id would write a row nothing
reconciles, which is worse than the event landing in the webhook ledger as an unknown type.
Verify the real payload against your account before that date, and watch the ledger for
`chargeback.received` in the meantime.
</Callout>
